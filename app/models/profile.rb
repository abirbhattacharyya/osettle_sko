class Profile < ActiveRecord::Base
  ...
  has_attached_file :logo,
                    :styles => { :thumb => "169x53>"},
                    :path => ':rails_root/public/profile/:attachment/:id/:style/:basename.:extension',
                    :url => '/:class/:id/:attachment?style=:style'

  validates_presence_of :name, :message => "^Hey, bussiness name can't be blank"
  validates_presence_of :address1, :message => "^Hey, address line1 can't be blank"
...
#  validate :valid_address
  ...
  def validate_url
    if self.web_url and !self.web_url.strip.blank?
